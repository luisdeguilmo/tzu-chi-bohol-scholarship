...
function ActivityFormModal({ isOpen, setIsOpen, onSuccess }) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const response = await fetch(
                "http://localhost:8000/app/views/activities.php",
                {
                    method: "POST",
                    headers: {
                        "Content-Type": "application/json",
    ...
    return (
        <div>
            <button
                onClick={() => setIsOpen(true)}
                className="py-4 pl-4 pr-3 fixed bottom-10 xl:right-28 right-14 bg-gradient-to-r from-green-500 to-emerald-500 text-white rounded-full transition-colors flex justify-center items-center shadow-lg hover:from-green-600 hover:to-emerald-600"
            >
                <svg
                    xmlns="http://www.w3.org/2000/svg"
                    className="h-5 w-5 mr-1"
            ...
            {isOpen && (
                <div className="fixed z-50 inset-0 flex items-center justify-center bg-black bg-opacity-50">
                    <div className="relative w-[80%] md:w-[50%] lg:w-[30%] bg-white rounded-md shadow-lg overflow-hidden max-h-[90vh] overflow-y-auto">
                        <div className="w-full px-8 pt-8 flex justify-between items-center">
                            <h2 className="text-2xl text-gray-600">
                                Add Activity
                            </h2>
                            <button
                                type="button"
                                onClick={() => setIsOpen(false)}
                                className="px-4 py-2 absolute top-4 right-5 font-thin text-gray-600 text-3xl hover:text-gray-800"
                            >
                                &times;
                            </button>
                        </div>
                        ...
                        <form onSubmit={handleSubmit} className="px-8 pb-8">
                            ...
                            <div className="flex gap-2">
                                <label className="w-[50%] py-1 flex flex-col gap-[1px] text-gray-600 text-sm">
                                    Start Time
                                    <input
                                        type="time"
                                        required
