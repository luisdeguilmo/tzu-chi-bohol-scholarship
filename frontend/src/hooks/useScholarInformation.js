    ...
    const fetchScholarsInformation = async () => {
        try {
            ...
            const response = await axios.get(
                `${BASE_URL}app/api/scholar-information.php?tab=${tab}&status=${status}&municipality=${municipality}&school=${school}&course=${course}&year_level=${yearLevel}&school_year=${schoolYear}&sort=${sortBy}`
            );
