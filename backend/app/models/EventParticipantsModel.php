...
class EventParticipantsModel {
    ...
    public function addParticipant($eventId, $userId) {
        $query = "INSERT INTO event_participants (event_id, account_id) VALUES (:event_id, :user_id)";
    ...
    public function setScholarAsAttended($eventId, $accountId) {
        $query = "UPDATE event_participants SET is_attended = '1' WHERE event_id = :event_id AND account_id = :account_id";
        $stmt = $this->pdo->prepare($query);
        $stmt->bindParam(':event_id', $eventId);
    ...
    public function removeParticipant($eventId, $userId) {
        $query = "DELETE FROM event_participants WHERE event_id = :event_id AND account_id = :user_id";
        $stmt = $this->pdo->prepare($query);
        $stmt->bindParam(':event_id', $eventId);
        $stmt->bindParam(':user_id', $userId);
        return $stmt->execute();
    ...
    public function checkUserParticipation($eventId, $scholarId) {
        $query = "SELECT COUNT(*) FROM event_participants WHERE event_id = :event_id AND account_id = :account_id";
        $stmt = $this->pdo->prepare($query);
